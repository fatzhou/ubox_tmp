...
export class SelectAudioVideoPage {
	...
	ionViewDidLoad() {
		GlobalService.consoleLog('ionViewDidLoad123 SelectAudioVideoPage');
	...
	loadImageError(file) {
		console.log("图片加载失败....." + file.thumbnailURL);
		file.thumbnailURL = "";
		file.loaded = true;
	}

	loadImageSuccess(file) {
		console.log("图片家在成功" + file.thumbnailURL);
		file.loaded = true;
	}
