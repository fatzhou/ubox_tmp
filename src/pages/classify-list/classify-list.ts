		...
		return this.http.post(url, {
			...
			.then((res: any) => {
				...
				if (res.err_no === 0) {
					...
					if (this.classify == 2) {
						list.forEach(item => {
							let md5 = Md5.hashStr(item.path + "/" + name).toString();
							let url = this.global.getBoxApi('downloadFile');
							let path = url + this.http.toQueryString({
								fullpath: item.path.replace(/\/$/, '') + '/' + item.name,
								disk_uuid: this.global.currDiskUuid,
	...
	goNextFolder(info) {
		if (this.allBtnsShow) {
			...
			if (info.fileStyle == 'image') {
				...
				let url = this.global.getBoxApi('downloadFile');
				let path = url + this.http.toQueryString({
					fullpath: info.path.replace(/\/$/, '') + '/' + info.name,
					disk_uuid: this.global.currDiskUuid,
					token: this.http.getCookieString(url).replace('token=', '')
				})
				// console.log("即将播放视频:" + info.videoId)

				// let videoWrapper = this.videos._results.find(item => {
				// 	return item.nativeElement.id == info.videoId
				// });
				// if (videoWrapper) {
				// 	let video: HTMLVideoElement = videoWrapper.nativeElement;
				// 	console.log("找到video，开始播放")
				// 	video.play()
				// 		.then(res => {
				// 			console.log("播放成功...")
				// 		})
